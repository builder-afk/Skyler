...
ICON_MAP = {
    ...
    "Startup Planning": "Rocket",
    "Financial Planning": "DollarSign",
    "Goal Planning": "Target",
    "Research": "Search",
    "Prioritization": "ListOrdered",
    "Risk Planning": "ShieldAlert",
    "Problem Solving": "Puzzle",
    "Team Planning": "Users",
    "Personal Life Planning": "Home",
    "AI Decision Assistant": "Bot",
    "Product Management": "Package",
    "Documentation": "FileText",
    "Thinking Tools": "Lightbulb",
    "Travel & Event Planning": "Map",
    "Premium Wow Prompts": "Sparkles"
}

def clean_title(title):
    return title.strip().replace('📊 ', '').replace('🧠 ', '').replace('📅 ', '').replace('🚀 ', '').replace('💰 ', '').replace('🎯 ', '').replace('📚 ', '').replace('📈 ', '').replace('⚖️ ', '').replace('🧩 ', '').replace('👥 ', '').replace('🏠 ', '').replace('🤖 ', '').replace('📦 ', '').replace('📑 ', '').replace('🌍 ', '').replace('⭐ ', '')

with open('prompts2.txt', 'r', encoding='utf-8') as f:
...
out.append("import { Briefcase, Target, Scale, Brain, Calendar, Rocket, DollarSign, Search, ListOrdered, ShieldAlert, Puzzle, Users, Home, Bot, Package, FileText, Lightbulb, Map, Sparkles } from 'lucide-react';\n")

cat_names = [c["name"] for c in categories]
out.append(f"export const NEW_CATEGORIES_2 = {json.dumps(cat_names)};\n")

out.append("export const NEW_TEMPLATES_BY_CATEGORY_2: Record<string, TemplateSection[]> = {")
...
with open('src/lib/data/new-templates2.tsx', 'w', encoding='utf-8') as f:
    f.write('\n'.join(out))
